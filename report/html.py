...
def _finding_html(f) -> str:
    ...
    if f.affected_items:
        bits.append(
            f'<p class="fix"><em>Affected:</em> {html.escape(_affected(f.affected_items))}</p>'
        )
    if f.remediation_hint:
        bits.append(
            f'<p class="fix"><em>How to check / fix:</em> {html.escape(f.remediation_hint)}</p>'
        )
    bits.append("</div>")
    return "\n".join(bits)
...
def _inventory_html(result: AuditResult) -> str:
    ...
    row(
        "Vendors",
        ", ".join(f"{v['name']} ({v['category']})" for v in sig.get("vendors", [])) or "none",
    )
    row(
        "Ecommerce / enhanced conv. / custom HTML",
    ...
    inv = (
        f"<h2>Measurement inventory</h2><div class='overflow'><table>{''.join(rows)}</table></div>"
    )
    if events:
        inv += (
            f"<h3>GA4 events configured ({len(events)})</h3><p class='muted'>"
            + ", ".join(f"<code>{html.escape(e)}</code>" for e in events)
            + "</p>"
        )
    return inv
...
def render_html(result: AuditResult, generated_at: str = "") -> str:
    ...
    counts = "".join(f'<span class="chip {k}">{k}={v}</span>' for k, v in r.scores.counts.items())
